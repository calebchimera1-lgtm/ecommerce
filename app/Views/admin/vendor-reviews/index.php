...
<h4 class="mb-4" style="color:#f8f7f4;">Vendor Reviews</h4>

<form method="GET" action="/admin/vendor-reviews" class="row g-2 mb-4">
    <div class="col-md-3">
    ...
    <table class="table table-dark table-hover align-middle">
        ...
        <tbody>
            ...
            <?php foreach ($reviews as $review): ?>
                <tr>
                    <td><a href="/store/<?= e($review['vendor_slug']) ?>" class="text-white" target="_blank"><?= e($review['vendor_store_name']) ?></a></td>
                    ...
                    <td class="text-end">
                        <?php if ((int) $review['is_approved'] !== 1): ?>
                            <form method="POST" action="/admin/vendor-reviews/<?= (int) $review['id'] ?>/approve" class="d-inline">
                                <?= csrf_field() ?>
                                <button type="submit" class="btn btn-sm btn-outline-success">Approve</button>
                            </form>
                        <?php endif; ?>
                        <form method="POST" action="/admin/vendor-reviews/<?= (int) $review['id'] ?>/delete" class="d-inline" onsubmit="return confirm('Remove this review?');">
                            <?= csrf_field() ?>
